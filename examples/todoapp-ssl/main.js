...
(function main() {
        ...
        var server = todo.createServer({
                ca: fs.readFileSync(options.ca || 'test-ca.crt'),
                cert: fs.readFileSync(options.cert || 'test.crt'),
                key: fs.readFileSync(options.key || 'test.key'),
                directory: dir,
                log: LOG
        });

        // At last, let's rock and roll
        server.listen((options.port || 8443), function onListening() {
